...
public class UserService implements IUserService {
    ...
    public Response setUserRole(String userId, String role) {
        Response response = new Response();
        try {
            if (role == null || (!role.equals("ADMIN") && !role.equals("STAFF") && !role.equals("USER"))) {
                response.setStatusCode(400);
                response.setMessage("Role không hợp lệ. Chỉ chấp nhận: ADMIN, STAFF, USER");
                return response;
            }
            ...
            userRepository.save(user);
            response.setStatusCode(200);
            response.setMessage("Đã cập nhật role thành " + role);
        } catch (OurException e) {
            response.setStatusCode(404);
            response.setMessage(e.getMessage());
        } catch (Exception e) {
