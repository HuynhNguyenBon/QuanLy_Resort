...
const VNPayReturnPage = () => {
  ...
  return (
    <div style={styles.container}>
      <div style={{...styles.card, borderTop: `4px solid ${isSuccess ? '#22c55e' : '#ef4444'}`}}>
        
        <div style={styles.iconWrap}>
          {isSuccess ? (
            <span style={{...styles.icon, background: '#dcfce7', color: '#16a34a'}}>✓</span>
          ) : (
            <span style={{...styles.icon, background: '#fee2e2', color: '#dc2626'}}>✗</span>
          )}
        </div>

        <h2 style={{color: isSuccess ? '#16a34a' : '#dc2626', margin: '12px 0'}}>
...
const styles = {
  ...
  btnSecondary: {
    padding: '12px 24px', background: '#e5e7eb',
