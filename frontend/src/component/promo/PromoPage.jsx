...
const PromoPage = () => {
  ...
  const openModal = (promo) => {
    ...
    if (promo.weekendOnly) {
      ...
      setCheckIn(
        promo.minDaysAdvance > 0 ? addDays(promo.minDaysAdvance) : addDays(1),
      );
      setCheckOut(
        promo.minNights > 1
          ? addDays(promo.minDaysAdvance + promo.minNights)
          : addDays(promo.minDaysAdvance + 2),
      );
    }
    setModal(promo);
  };
      ...
      {modal && (
        <div className="promo-modal-bg" onClick={() => setModal(null)}>
          <div className="promo-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {modal.hint && (
              <div className="promo-modal-hint">
                {t("hintLabel")} {modal.hint}
              </div>
            )}
            ...
            <div className="promo-modal-body">
              <div className="promo-modal-row">
                <div className="promo-modal-field">
                  ...
                  <input
                    ...
                    value={checkIn}
                    min={
                      modal.minDaysAdvance > 0
                        ? addDays(modal.minDaysAdvance)
                        : addDays(1)
                    }
                    onChange={(e) => {
                      ...
                      setError("");
                      if (modal.weekendOnly)
                        setCheckOut(getSundayAfter(e.target.value));
                    }}
                  />
                </div>
                <div className="promo-modal-field">
              ...
              {nights > 0 && (
                <div className="promo-modal-nights">
                  ...
                  {nights >= modal.minNights ? (
                    ...
                    <span className="promo-nights-fail">
                      {" "}
                      · {t("validation.minNights", { n: modal.minNights })}
                    </span>
                  )}
                </div>
              )}
              ...
              <div className="promo-modal-conditions">
                <div className="promo-modal-conditions-title">
                  {t("conditionsLabel")}
                </div>
                <ul>
                  {modal.conditions.map((c, i) => (
                    <li key={i}>✓ {c}</li>
                  ))}
                </ul>
              </div>
