  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchType) {
        ...
        // Không chọn loại -> gọi API lấy tất cả phòng available
        try {
          const res = await ApiService.getAllAvailableRooms();
          availableRooms = res.roomList || [];
          // Lọc theo ngày bằng cách gọi từng loại nếu getAllAvailableRooms không filter được
          if (availableRooms.length === 0 && roomTypes.length > 0) {
            const firstType = roomTypes[0];
            const sample = await ApiService.getAvailableRoomsByDateAndType(start, end, firstType);
            if ((sample.roomList || []).length >= 0) {
              // API hỗ trợ filter date -> gọi từng loại tuần tự (không dùng map song song)
              const collected = [];
              for (const type of roomTypes) {
                try {
                  const r = await ApiService.getAvailableRoomsByDateAndType(start, end, type);
                  collected.push(...(r.roomList || []));
                } catch { /* bỏ qua lỗi từng loại */ }
              }
              availableRooms = collected;
            }
          }
        } catch (e) {
