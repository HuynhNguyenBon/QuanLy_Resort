...
const WalkInBookingPage = () => {
  ...
  const handleSearch = async () => {
    if (!checkIn || !checkOut || new Date(checkIn) >= new Date(checkOut)) {
      showMsg(t("walkIn.errorDate"), false);
      return;
    }
    setLoading(true);
    try {
      ...
      const results = await Promise.all(
        types.map((t) =>
          ApiService.getAvailableRoomsByDateAndType(checkIn, checkOut, t)
            .then((r) => r.roomList || [])
            .catch(() => []),
        ),
      );
      ...
      if (available.length === 0) {
        showMsg(t("walkIn.errorNoRoom"), false);
      } else {
        setStep(2);
      }
    } catch (e) {
  ...
  const handleGuestNext = () => {
    if (!guestName.trim()) {
      showMsg(t("walkIn.errorName"), false);
      return;
    }
    if (!guestEmail.trim()) {
      showMsg(t("walkIn.errorEmail"), false);
      return;
    }
    setStep(4);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const found = users.find(
        (u) => u.email?.toLowerCase() === guestEmail.trim().toLowerCase(),
      );
      ...
      const res = await ApiService.bookRoom(
        selectedRoom.id,
        found.id,
        bookingData,
      );
  ...
  return (
    <div className="adm-dashboard">
              ...
              {i < STEP_LABELS.length - 1 && (
                <div
                  style={{
                    flex: 1,
                    height: 2,
                    background: step > i + 1 ? "#0d9488" : "#f1f5f9",
                    margin: "0 8px",
                    marginBottom: 22,
      ...
      <div
        className="adm-section"
        style={{ padding: 28, borderRadius: "0 0 12px 12px", minHeight: 360 }}
      >
        {/* ── STEP 1: Dates ── */}
        {step === 1 && (
          <div
          ...
          >
            ...
            {checkIn && checkOut && new Date(checkIn) < new Date(checkOut) && (
              ...
              >
                🌙 {nights} {t("walkIn.nightsSummary")} · {adults}{" "}
                {t("walkIn.adults")}
                {children > 0 ? `, ${children} ${t("walkIn.children")}` : ""}
              </div>
            )}
            <button
              ...
              style={{
                ...
                border: "none",
                width: "100%",
                backgroundColor: loading ? "#99d6d0" : "#0d9488",
                color: "#fff",
                fontWeight: 700,
                fontSize: "0.95rem",
              ...
              onMouseEnter={(e) => {
                if (!loading) e.currentTarget.style.backgroundColor = "#0a7c73";
              }}
              onMouseLeave={(e) => {
        ...
        {step === 2 && (
          <div>
            ...
            >
              <h3 style={{ margin: 0, fontWeight: 700, color: "#1a1a2e" }}>
                🛏️ {t("walkIn.step2Title")} ({rooms.length}{" "}
                {t("walkIn.available")})
              </h3>
              <button
                onClick={() => setStep(1)}
              ...
              {rooms.map((room) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      = {formatPrice(room.roomPrice * nights, lang)}{" "}
                      {t("walkIn.totalNight")}
                    </span>
                  </div>
                  {(getRoomTranslation(room.roomType, lang)?.roomDescription ||
                    ...
                    >
                      {(() => {
                        ...
                        return d.slice(0, 80) + (d.length > 80 ? "..." : "");
                      })()}
                    </p>
                  )}
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {[
                    { key: "cash", label: t("walkIn.cash") },
                    { key: "vnpay", label: t("walkIn.vnpay") },
                  ].map((p) => (
                    <button
                      key={p.key}
                      ...
                      style={{
                        ...
                        fontWeight: payMethod === p.key ? 700 : 400,
                        cursor: "pointer",
                        fontSize: "0.9rem",
                      }}
                    >
        ...
        {step === 4 && selectedRoom && (
          <div style={{ width: "100%" }}>
            ...
            {[
              {
                icon: "🛏️",
                label: t("walkIn.fieldRoom"),
                value: `#${selectedRoom.id} — ${selectedRoom.roomType}`,
              },
              { icon: "📅", label: t("walkIn.checkin"), value: checkIn },
              { icon: "📅", label: t("walkIn.checkout"), value: checkOut },
              {
                icon: "🌙",
                label: t("walkIn.fieldNights"),
                value: `${nights} ${t("walkIn.nightsSummary")}`,
              },
              {
                icon: "👥",
                label: t("walkIn.fieldGuests"),
                value: `${adults} ${t("walkIn.adults")}${children > 0 ? `, ${children} ${t("walkIn.children")}` : ""}`,
              },
              {
                icon: "💰",
                label: t("walkIn.fieldTotal"),
                value: formatPrice(selectedRoom.roomPrice * nights, lang),
                accent: true,
              },
              ...
              {
                icon: "💳",
                label: t("walkIn.fieldPay"),
                value: payMethod === "cash" ? t("walkIn.cashLabel") : "VNPay",
              },
              ...
              >
                <span style={{ color: "#64748b", fontSize: "0.87rem" }}>
                  {icon} {label}
                </span>
                <span
                  style={{
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10,
                border: "none",
                background: submitting ? "#99d6d0" : "#0d9488",
                color: "#fff",
                fontWeight: 700,
                fontSize: "1rem",
