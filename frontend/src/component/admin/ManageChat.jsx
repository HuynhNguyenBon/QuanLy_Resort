...
const ManageChat = () => {
  ...
  const handleDeleteChat = (userId) => {
    if (window.confirm(t("manageChatPage.deleteConfirm", { user: userId }))) {
  ...
  return (
    <div className="manage-chat-container">
      <div className="chat-sidebar">
        <div className="sidebar-header">📩 {t("manageChatPage.messages")}</div>

        <div className="user-list">
          {Object.keys(chats).map((id) => (
            <div
              key={id}
              className={`user-item ${activeUser === id ? "active" : ""}`}
      ...
      <div className="chat-main">
        {activeUser ? (
          <>
            <div className="messages-v3">
              {Object.values(chats[activeUser])
                ...
                .map((m, i) => (
                  <div
                    key={i}
                    className={`message-row ${m.sender === "admin" ? "admin" : "user"}`}
                  >
                    <div className="message-bubble-v3">{m.text}</div>
                  </div>
                ))}
            ...
            <form className="admin-input-area" onSubmit={sendReply}>
              <input
                ...
                placeholder={t("manageChatPage.replyPlaceholder")}
              />
          ...
          <div className="no-chat-selected">
            {t("manageChatPage.selectCustomer")}
          </div>
        )}
      </div>
    </div>
