...
const AdminLayout = () => {
  ...
  const today = new Date().toLocaleDateString(
    i18n.language === "ja" ? "ja-JP" : i18n.language === "vi" ? "vi-VN" : "en-US",
  ...
  const handleLogout = () => { ApiService.logout(); navigate("/login"); };
    ...
    <div className="adm-layout">
      ...
      <div className="adm-main">
        <div className="adm-topbar">
          ...
          <div className="adm-topbar-right" style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div ref={langRef} style={{ position: "relative" }}>
              <button onClick={() => setLangOpen(o => !o)}
                ...
                <img src={`https://flagcdn.com/w40/${currentLang.country}.png`}
                  width="18" height="12" alt={currentLang.code} style={{ borderRadius: 2 }} />
                <span>{currentLang.code.toUpperCase()}</span>
                <span style={{ fontSize: "0.65rem", color: "#aaa",
                  transform: langOpen ? "rotate(180deg)" : "none", transition: "0.2s" }}>▼</span>
              </button>

              {langOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0,
                  background: "#fff", borderRadius: 10, border: "1.5px solid #e2e8f0",
                  boxShadow: "0 8px 24px rgba(0,0,0,0.12)", zIndex: 999, overflow: "hidden", minWidth: 140 }}>
                  {LANGS.map(lang => (
                    <div key={lang.code}
                      onClick={() => { i18n.changeLanguage(lang.code); setLangOpen(false); }}
                      style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px",
                        cursor: "pointer", fontSize: "0.85rem",
                        background: lang.code === i18n.language ? "#f0fdfa" : "transparent",
                        color: lang.code === i18n.language ? "#0d9488" : "#1a1a2e",
                        fontWeight: lang.code === i18n.language ? 700 : 400 }}
                      onMouseEnter={e => { if (lang.code !== i18n.language) e.currentTarget.style.background = "#f8fafc"; }}
                      onMouseLeave={e => { e.currentTarget.style.background = lang.code === i18n.language ? "#f0fdfa" : "transparent"; }}>
                      <img src={`https://flagcdn.com/w40/${lang.country}.png`}
                        width="20" height="13" alt={lang.code} style={{ borderRadius: 2 }} />
                      {lang.name}
                      {lang.code === i18n.language && <span style={{ marginLeft: "auto", color: "#0d9488" }}>✓</span>}
                    </div>
                  ))}
                </div>
              )}
