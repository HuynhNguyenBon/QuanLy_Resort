...
const ManageServicesPage = () => {
  ...
  const handleSave = async () => {
    ...
    if (!name.trim()) {
      setModalErr("Tên dịch vụ không được để trống.");
      return;
    }
    if (!description.trim()) {
      setModalErr("Mô tả không được để trống.");
      return;
    }
    setModalErr("");
    setSaving(true);
  ...
  const handleDelete = async (s) => {
    ...
    try {
      ...
      showMsg(
        "Xoá thất bại: " + (e.response?.data?.message || e.message),
        false,
      );
    } finally {
      setDeletingId(null);
