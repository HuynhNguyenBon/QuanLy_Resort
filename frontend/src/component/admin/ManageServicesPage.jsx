...
const ManageServicesPage = () => {
  ...
  const fetchServices = () => {
    ...
    ApiService.getAllServices()
      .then(r => {
        ...
        const apiPaid = apiList.filter(s => s.price != null && Number(s.price) > 0)
                               .map(s => ({ ...s, _isApi: true }));
        const apiNames = new Set(apiList.map(s => s.name));
        const staticExtra = STATIC_SERVICES
          .filter(s => !apiNames.has(s.name) && !hidden.includes(s.name))
          .map(s => ({ ...s, _isApi: false }));
        setServices([...apiPaid, ...staticExtra]);
      })
      .catch(() => {
        const hidden = getHidden();
        setServices(STATIC_SERVICES.filter(s => !hidden.includes(s.name)).map(s => ({ ...s, _isApi: false })));
      })
  ...
  const handleSave = async () => {
    ...
    if (!name.trim()) { setModalErr("Tên dịch vụ không được để trống."); return; }
    if (!description.trim()) { setModalErr("Mô tả không được để trống."); return; }
    setModalErr("");
    setSaving(true);
    ...
    try {
      ...
      if (modal.mode === "add" || isBuiltIn) {
        // Built-in service: tạo mới vào backend, ẩn static version
        await ApiService.addService(payload);
        if (isBuiltIn) addHidden(modal.data.name);
        showMsg(isBuiltIn ? "Đã lưu dịch vụ vào hệ thống." : "Đã thêm dịch vụ thành công.");
      } else {
        await ApiService.updateService(modal.data.id, payload);
        showMsg("Đã cập nhật dịch vụ.");
      }
      closeModal();
      fetchServices();
    } catch (e) {
      const detail = e.response?.data?.message
        ...
        || e.message
        || "Lỗi không xác định";
      setModalErr("Lỗi " + (e.response?.status || "") + ": " + detail);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (s) => {
    if (!window.confirm("Xác nhận xoá dịch vụ này?")) return;
    setDeletingId(s.id);
    try {
      if (!s._isApi) {
        // Built-in: chỉ ẩn khỏi danh sách
        addHidden(s.name);
        setServices(prev => prev.filter(x => x.id !== s.id));
        showMsg("Đã ẩn dịch vụ khỏi danh sách.");
      } else {
        await ApiService.deleteService(s.id);
        setServices(prev => prev.filter(x => x.id !== s.id));
        showMsg("Đã xoá dịch vụ.");
      }
    } catch (e) {
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          <button onClick={() => openEdit(s)}
            style={{ padding: "5px 11px", borderRadius: 7, border: "1px solid #0d9488",
          ...
          <button onClick={() => handleDelete(s)} disabled={deletingId === s.id}
            style={{ padding: "5px 11px", borderRadius: 7, border: "1px solid #fca5a5",
              background: "#fff5f5", color: "#e74c3c", cursor: "pointer",
              fontSize: "0.78rem", fontWeight: 600, opacity: deletingId === s.id ? 0.6 : 1 }}
            onMouseEnter={e => { if (deletingId !== s.id) { e.target.style.background = "#e74c3c"; e.target.style.color = "#fff"; }}}
  ...
  const fieldStyle = {
    ...
    border: "1.5px solid #e8ecef", fontSize: "0.88rem", outline: "none",
    background: "#fafbfd", boxSizing: "border-box", fontFamily: "inherit",
          ...
          {services.filter(s => Number(s.price) > 0).length > 0 && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
                {services.filter(s => Number(s.price) > 0).map(s => (
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                <button onClick={handleSave} disabled={saving}
                  style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "none",
                    background: saving ? "#99d6d0" : "#0d9488", color: "#fff",
