  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setRoomPopup(p => ({ ...p, editMode: false,
        booking: { ...p.booking, ...editForm,
          totalNumOfGuest: Number(editForm.numOfAdults) + Number(editForm.numOfChildren) }
      }));
    } catch (err) {
      console.error("updateBooking error:", err?.response?.data || err?.message || err);
      const detail = err?.response?.data?.message || err?.message || "";
      setSaveMsg(`❌ Lưu thất bại${detail ? ": " + detail : ", thử lại."}`);
    } finally {
      setSaving(false);
    }
  };
                    ...
                    return (
                      <div key={room.id}
                        ...
                        onClick={() => occupied
                          ...
                          : ApiService.isAdmin() && navigate(`/admin/edit-room/${room.id}`)}>
      ...
      {roomPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000,
          ...
          <div style={{ background: "#fff", borderRadius: 16, width: "100%",
            ...
            {/* Header */}
            <div style={{ background: roomPopup.detailMode
                ? "linear-gradient(135deg, #0d9488, #0f766e)"
                : "linear-gradient(135deg, #e74c3c, #c0392b)",
              padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {roomPopup.detailMode && (
                  <button onClick={() => setRoomPopup(p => ({ ...p, detailMode: false }))}
                    style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "#fff",
                      width: 28, height: 28, borderRadius: "50%", cursor: "pointer", fontSize: "0.85rem",
                ...
                <div style={{ color: "#fff" }}>
                  ...
                  <div style={{ fontSize: "0.8rem", opacity: 0.85, marginTop: 2 }}>
                    {roomPopup.detailMode ? `📋 ${t("editBooking.title")}` : `🔴 ${t("overview.occupied_label")}`}
                  </div>
                </div>
              </div>
              ...
              <div style={{ maxHeight: "70vh", overflowY: "auto" }}>
                ...
                {roomPopup.room.roomPhotoUrl && (
                  <div style={{ position: "relative", height: 200, overflow: "hidden" }}>
                    <img src={roomPopup.room.roomPhotoUrl} alt={roomPopup.room.roomType}
                  ...
                  {roomPopup.booking && (
                    <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 10,
                      ...
                      <div>
                        ...
                        <div style={{ fontWeight: 800, color: "#0d9488", fontSize: "1rem", letterSpacing: 1 }}>
                  ...
                  {roomPopup.booking && (
                    roomPopup.editMode ? (
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {saveMsg && (
                          <div style={{ padding: "8px 12px", borderRadius: 8, fontSize: "0.85rem", fontWeight: 600,
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                          {[
                            { key: "checkInDate",  label: "📅 " + t("bookings.cols.checkin"),  type: "date" },
                            { key: "checkOutDate", label: "📅 " + t("bookings.cols.checkout"), type: "date" },
                            { key: "numOfAdults",  label: "🧑 Người lớn",  type: "number", min: 1 },
                            { key: "numOfChildren",label: "👶 Trẻ em",     type: "number", min: 0 },
                          ].map(({ key, label, type, min }) => (
                            <div key={key}>
                              <div style={{ fontSize: "0.68rem", color: "#94a3b8", fontWeight: 700,
                                textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
                              <input type={type} min={min} value={editForm[key] ?? ""}
                        ...
                        <div>
                          <div style={{ fontSize: "0.68rem", color: "#94a3b8", fontWeight: 700,
                            textTransform: "uppercase", marginBottom: 4 }}>📊 {t("bookings.cols.status")}</div>
                          <select value={editForm.bookingStatus ?? ""}
                            onChange={e => setEditForm(f => ({ ...f, bookingStatus: e.target.value }))}
                            style={{ width: "100%", padding: "8px 10px", borderRadius: 8,
                              border: "1.5px solid #e2e8f0", fontSize: "0.88rem",
                              fontFamily: "inherit", outline: "none", background: "#fff" }}>
                            {["BOOKED","CONFIRMED","CHECKED_IN","CHECKED_OUT","CANCELLED"].map(s => (
                        ...
                        <div style={{ borderTop: "1px dashed #e2e8f0", paddingTop: 10,
                          ...
                          {[
                            ...
                            <div key={label} style={{ fontSize: "0.82rem", color: "#64748b", display: "flex", gap: 6 }}>
                              <span>{icon}</span>
                      ...
                      <>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                          {[
                            ...
                            { icon: "📊", label: t("bookings.cols.status"),
                              value: roomPopup.booking.bookingStatus || roomPopup.booking.status || "—" },
                          ].map(({ icon, label, value }) => (
                            <div key={label} style={{ background: "#f8fafc", borderRadius: 10, padding: "12px 14px" }}>
                              <div style={{ fontSize: "0.7rem", color: "#94a3b8", fontWeight: 700,
                                textTransform: "uppercase", marginBottom: 4 }}>{icon} {label}</div>
                        ...
                        <div style={{ borderTop: "1px solid #f0f0f0", paddingTop: 14,
                          ...
                          {[
                            ...
                            <div key={label} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <span style={{ width: 30, height: 30, borderRadius: 8, background: "#f1f5f9",
                                display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                {icon}
                              </span>
                              <div>
                                <div style={{ fontSize: "0.68rem", color: "#94a3b8", fontWeight: 600, textTransform: "uppercase" }}>{label}</div>
                                <div style={{ fontWeight: 600, color: "#1a1a2e", fontSize: "0.85rem" }}>{value || "—"}</div>
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            <div style={{ padding: "14px 24px", borderTop: "1px solid #f0f2f5",
              ...
              {roomPopup.editMode ? (
                <>
                  <button onClick={() => setRoomPopup(p => ({ ...p, editMode: false }))}
                    disabled={saving}
                ...
                <button onClick={() => setRoomPopup(p => ({ ...p, detailMode: true }))}
