...
const ROLES = ["Lễ tân", "Quản lý", "Buồng phòng", "Bảo vệ", "Bếp", "Kỹ thuật", "Spa", "Khác"];
const EMPTY = { name: "", role: "", phone: "", email: "", startDate: "", note: "" };

const getStaff = () => { try { return JSON.parse(localStorage.getItem(STAFF_KEY) || "[]"); } catch { return []; } };
const saveStaff = (list) => localStorage.setItem(STAFF_KEY, JSON.stringify(list));

// meta: { [apiId]: { role, startDate, note, phone } }
const getMeta = () => { try { return JSON.parse(localStorage.getItem(STAFF_META_KEY) || "{}"); } catch { return {}; } };
const saveMeta = (meta) => localStorage.setItem(STAFF_META_KEY, JSON.stringify(meta));

const COLORS = ["#6366f1","#0d9488","#f59e0b","#e74c3c","#8b5cf6","#06b6d4","#ec4899","#14b8a6"];
...
const ManageStaffPage = () => {
  ...
  const reloadStaff = () => {
    ...
    ApiService.getAllUsers()
      .then(r => {
        ...
        const localExtra = local.filter(s => !s.email || !apiEmails.has(s.email));
  ...
  const openAdd  = () => { setModalErr(""); setModal({ mode: "add",  data: { ...EMPTY } }); };
  const openEdit = (s) => { setModalErr(""); setModal({ mode: "edit", data: { ...s } }); };
  ...
  const handleDelete = (id) => {
    ...
    saveStaff(next);
    showMsg("Đã xoá nhân viên.");
    reloadStaff();
  };

  const openAccountModal = (s) => {
    setAccountErr("");
    setAccountModal({ staffId: s.id, name: s.name, email: s.email || "", password: "", confirmPassword: "" });
  };

  const handleCreateAccount = async () => {
    const { name, email, password, confirmPassword } = accountModal;
    if (!email.trim())       { setAccountErr("Email không được để trống."); return; }
    if (!password)           { setAccountErr("Mật khẩu không được để trống."); return; }
    if (password.length < 6) { setAccountErr("Mật khẩu phải có ít nhất 6 ký tự."); return; }
    if (password !== confirmPassword) { setAccountErr("Mật khẩu xác nhận không khớp."); return; }
    setAccountSaving(true);
    try {
      await ApiService.registerUser({ name, email: email.trim(), password });
      showMsg(`Đã tạo tài khoản cho ${name}. Email: ${email}`);
      const localList = getStaff();
      const updated = localList.map(s => s.id === accountModal.staffId ? { ...s, email: email.trim(), hasAccount: true } : s);
      saveStaff(updated);
      setAccountModal(null);
      reloadStaff();
    } catch (e) {
      setAccountErr(e.response?.data?.message || e.message || "Tạo tài khoản thất bại.");
    } finally {
  ...
  const roleCounts = ROLES.reduce((acc, r) => ({ ...acc, [r]: staff.filter(s => s.role === r).length }), {});

  const fieldStyle = { width: "100%", padding: "9px 13px", borderRadius: 8, border: "1.5px solid #e8ecef",
    fontSize: "0.88rem", outline: "none", background: "#fafbfd", boxSizing: "border-box", fontFamily: "inherit" };
  const labelStyle = { display: "block", marginBottom: 5, fontWeight: 600, fontSize: "0.79rem", color: "#6b7280" };
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={s.id}
                style={{ borderBottom: "1px solid #f0f2f5", background: idx % 2 === 0 ? "#fff" : "#fafbfd", transition: "background 0.12s" }}
                onMouseEnter={e => e.currentTarget.style.background = "#f0fdf4"}
                onMouseLeave={e => e.currentTarget.style.background = idx % 2 === 0 ? "#fff" : "#fafbfd"}>

                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: colorFor(s.name),
                    ...
                    <div>
                      ...
                      {s.note && <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>{s.note}</div>}
                    </div>
                  </div>
                </td>
                <td style={{ padding: "14px 16px" }}>
                  <span style={{ padding: "4px 10px", borderRadius: 20, fontSize: "0.78rem", fontWeight: 600,
                    background: "#f0fdfa", color: "#0d9488", border: "1px solid #ccfbf1" }}>{s.role}</span>
                </td>
                <td style={{ padding: "14px 16px", color: "#475569" }}>{s.phone || "—"}</td>
                <td style={{ padding: "14px 16px", color: "#475569" }}>{s.email || "—"}</td>
                <td style={{ padding: "14px 16px", color: "#475569" }}>{s.startDate || "—"}</td>
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <button onClick={() => openEdit(s)}
                      style={{ padding: "5px 12px", borderRadius: 7, border: "1px solid #0d9488",
                    ...
                    {s.hasAccount ? (
                      ...
                      <button onClick={() => openAccountModal(s)}
                        style={{ padding: "5px 12px", borderRadius: 7, border: "1px solid #6366f1",
                          background: "transparent", color: "#6366f1", cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}
                    ...
                    <button onClick={() => handleDelete(s.id)}
                      style={{ padding: "5px 12px", borderRadius: 7, border: "1px solid #fca5a5",
                        background: "#fff5f5", color: "#e74c3c", cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}
            ...
            <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                <div>
                  ...
                  <input value={modal.data.email} onChange={e => setModal(m => ({ ...m, data: { ...m.data, email: e.target.value } }))}
                    placeholder="nhanvien@gmail.com" style={fieldStyle}
                    onFocus={e => e.target.style.borderColor = "#0d9488"} onBlur={e => e.target.style.borderColor = "#e8ecef"} />
                </div>
              </div>
              <div>
              ...
              {modalErr && (
                <div style={{ padding: "10px 14px", borderRadius: 9, background: "#fef2f2",
                  color: "#b91c1c", border: "1px solid #fecaca", fontSize: "0.85rem" }}>
                  ⚠️ {modalErr}
                </div>
              )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000,
          ...
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 440,
            ...
            <div style={{ background: "linear-gradient(135deg, #6366f1, #4f46e5)", padding: "18px 24px",
              borderRadius: "16px 16px 0 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ color: "#fff", fontWeight: 700, fontSize: "1rem" }}>🔑 Tạo tài khoản nhân viên</span>
            ...
            <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ padding: "10px 14px", borderRadius: 9, background: "#eef2ff",
                color: "#4338ca", fontSize: "0.83rem", border: "1px solid #c7d2fe" }}>
                ℹ️ Tạo tài khoản đăng nhập hệ thống cho <strong>{accountModal.name}</strong>.
                Nhân viên dùng email + mật khẩu này để đăng nhập vào trang nhân viên.
              </div>
              <div>
                <label style={labelStyle}>Email đăng nhập *</label>
                <input type="email" value={accountModal.email}
                  onChange={e => setAccountModal(m => ({ ...m, email: e.target.value }))}
                  placeholder="nhanvien@bbhh.com" style={fieldStyle}
                  onFocus={e => e.target.style.borderColor = "#6366f1"}
                  onBlur={e => e.target.style.borderColor = "#e8ecef"} />
              </div>
              <div>
                <label style={labelStyle}>Mật khẩu *</label>
                <input type="password" value={accountModal.password}
                  onChange={e => setAccountModal(m => ({ ...m, password: e.target.value }))}
                  placeholder="Tối thiểu 6 ký tự" style={fieldStyle}
                  onFocus={e => e.target.style.borderColor = "#6366f1"}
                  onBlur={e => e.target.style.borderColor = "#e8ecef"} />
              </div>
              <div>
                <label style={labelStyle}>Xác nhận mật khẩu *</label>
                <input type="password" value={accountModal.confirmPassword}
                  onChange={e => setAccountModal(m => ({ ...m, confirmPassword: e.target.value }))}
                  placeholder="Nhập lại mật khẩu" style={fieldStyle}
                  onFocus={e => e.target.style.borderColor = "#6366f1"}
                  onBlur={e => e.target.style.borderColor = "#e8ecef"} />
              </div>
              {accountErr && (
