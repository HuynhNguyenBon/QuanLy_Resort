...
const TypeDropdown = ({ options, value, onChange }) => {
  ...
  const selected = value || "Tất cả loại phòng";
    ...
    <div ref={ref} style={{ position: "relative", minWidth: 200 }}>
      <button
        ...
        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8,
          ...
          cursor: "pointer", fontSize: "0.875rem", fontWeight: isFiltered ? 600 : 400,
          outline: "none", transition: "all 0.15s" }}>
        <span>{selected}</span>
        <span style={{ fontSize: "0.7rem", color: isFiltered ? "#0d9488" : "#aab4be",
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0,
          ...
          {["", ...options].map(opt => {
            const label = opt || "Tất cả loại phòng";
            ...
            return (
              <div key={opt}
                ...
                style={{ padding: "10px 16px", cursor: "pointer", fontSize: "0.875rem",
                  ...
                  fontWeight: isActive ? 600 : 400, transition: "background 0.12s" }}
                onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = "#f0fdfa"; e.currentTarget.style.color = isActive ? "#fff" : "#0d9488"; }}
                onMouseLeave={e => { e.currentTarget.style.background = isActive ? "#0d9488" : "transparent"; e.currentTarget.style.color = isActive ? "#fff" : "#1a1a2e"; }}>
                {label}
              </div>
            );
          })}
  ...
  return (
    <div className="adm-dashboard">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "1.4rem", fontWeight: 700, color: "#1a1a2e" }}>🛏️ Quản lý phòng</h2>
          <p style={{ margin: "4px 0 0", color: "#888", fontSize: "0.88rem" }}>{rooms.length} phòng trong hệ thống</p>
        </div>
        <button className="adm-quick-btn" style={{ borderLeftColor: "#0d9488", flex: "none", fontWeight: 700 }}
      ...
      {/* Filters */}
      <div className="adm-section" style={{ padding: "16px 20px", marginBottom: 16 }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ position: "relative", minWidth: 240 }}>
            ...
            <input
              placeholder="Tìm theo loại phòng..."
              value={search}
              ...
              style={{ width: "100%", padding: "9px 14px 9px 36px", borderRadius: 10,
                ...
                background: "#f8fafc", color: "#1a1a2e", boxSizing: "border-box",
          ...
          {/* Clear filter */}
          {(search || selectedType) && (
            <button onClick={() => { setSearch(""); setSelectedType(""); setCurrentPage(1); }}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 14px",
                borderRadius: 10, border: "1.5px solid #fecaca", background: "#fff5f5",
                cursor: "pointer", fontSize: "0.85rem", color: "#e74c3c", fontWeight: 600,
                transition: "all 0.15s" }}
              onMouseEnter={e => { e.currentTarget.style.background = "#fef2f2"; e.currentTarget.style.borderColor = "#e74c3c"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "#fff5f5"; e.currentTarget.style.borderColor = "#fecaca"; }}>
              <span style={{ fontSize: "0.8rem" }}>✕</span> Xoá bộ lọc
            </button>
          )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.88rem" }}>
          <thead>
            <tr style={{ background: "#f7f9fc", borderBottom: "2px solid #e8ecef" }}>
              {["Ảnh", "ID", "Loại phòng", "Giá / đêm", "Mô tả", "Thao tác"].map(h => (
          ...
          <tbody>
            {paginated.length === 0 ? (
              ...
              <tr key={room.id}
                ...
                {/* ID */}
                <td style={{ padding: "10px 16px", color: "#888", fontWeight: 600 }}>
                  #{room.id}
                </td>

                {/* Loại phòng */}
                <td style={{ padding: "10px 16px" }}>
                  <span style={{ fontWeight: 700, color: "#1a1a2e" }}>{room.roomType}</span>
                </td>

                {/* Giá */}
                <td style={{ padding: "10px 16px" }}>
                  <span style={{ fontWeight: 700, color: "#0d9488" }}>${room.roomPrice}</span>
                </td>

                {/* Mô tả */}
                <td style={{ padding: "10px 16px", color: "#666", maxWidth: 280 }}>
                  <span title={room.roomDescription}>
                    {room.roomDescription
                      ? room.roomDescription.length > 60
                        ? room.roomDescription.slice(0, 60) + "..."
                        : room.roomDescription
                      : "—"}
                  </span>
                </td>
                ...
                <td style={{ padding: "10px 16px" }}>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button
                      ...
                      style={{ padding: "6px 14px", borderRadius: 7, border: "1px solid #0d9488",
                    ...
                    <button
                      ...
                      style={{ padding: "6px 14px", borderRadius: 7, border: "1px solid #e74c3c",
                        background: "transparent", color: "#e74c3c", cursor: "pointer",
                        fontSize: "0.82rem", fontWeight: 600, transition: "all 0.15s",
                        opacity: deletingId === room.id ? 0.6 : 1 }}
                      onMouseEnter={e => { if (deletingId !== room.id) { e.target.style.background = "#e74c3c"; e.target.style.color = "#fff"; }}}
                      onMouseLeave={e => { e.target.style.background = "transparent"; e.target.style.color = "#e74c3c"; }}>
                      {deletingId === room.id ? "..." : "🗑 Xoá"}
                    </button>
                  </div>
                </td>
              </tr>
