...
const ROLES_DISPLAY = {
  ...
  STAFF: { bg: "#ede9fe", color: "#7c3aed", border: "#c4b5fd" },
...
const ManageUsersPage = () => {
  ...
  const handleSetRole = async (user, newRole) => {
    const label =
      newRole === "STAFF" ? t("users.roleStaff") : t("users.roleUser");
    if (
      !window.confirm(
        t("users.confirmSetRole", { name: user.name, role: label }),
      )
    )
      return;
    try {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      showMsg(t("users.deleteSuccess"));
    } catch (e) {
      showMsg(
        t("users.deleteFail") + ": " + (e.response?.data?.message || e.message),
        false,
      );
    } finally {
      setDeletingId(null);
          ...
          <p style={{ margin: "4px 0 0", color: "#888", fontSize: "0.88rem" }}>
            {
              users.filter(
                (u) =>
                  !["ADMIN", "STAFF"].includes((u.role || "").toUpperCase()),
              ).length
            }{" "}
            {t("users.subtitle")}
          </p>
          ...
          <thead>
            ...
            >
              {[
                ...
                t("users.cols.email"),
                t("users.cols.role"),
                t("users.cols.bookings"),
                ...(ApiService.isAdmin() ? [t("users.cols.actions")] : []),
              ].map((h) => (
                <th
                  key={h}
                  style={{
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 36,
                          borderRadius: "50%",
                          background: colorFor(user.id || idx),
                          color: "#fff",
                          display: "flex",
                          alignItems: "center",
                        ...
                        {user.role !== "ADMIN" && (
                          <>
                            {user.role === "STAFF" ? (
                              ...
                              <button
                                onClick={() => handleSetRole(user, "STAFF")}
                                style={{
                                  padding: "5px 10px",
                                  borderRadius: 7,
                                  border: "1px solid #c4b5fd",
